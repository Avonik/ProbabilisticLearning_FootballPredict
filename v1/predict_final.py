...
# === Konfiguration ===
TEAM1           = "Bayern Munich"   # erstes Team (oder "Heim" bei Heimspiel)
TEAM2           = "Stuttgart"       # zweites Team
NEUTRAL_VENUE   = False              # True = neutraler Platz (Pokalfinale Berlin)
...
def load_samples(cache: Path, season: str) -> tuple:
    ...
    df = load_bundesliga(2024, 2026, fallback_synthetic=False)
...
def predict_match(samples, L, team1: str, team2: str, neutral: bool) -> dict:
    for name in (team1, team2):
        if name not in L.teams:
            ...
            print(f"Verfügbare Teams: {sorted(L.teams)}")
            sys.exit(1)
        ...
        for x in range(MAX_GOALS + 1):
            for y in range(MAX_GOALS + 1):
                score_mat[x, y] += match_likelihood(
                    x, y, lam_x, lam_y, lam_avg, lam_avg
                )
...
def plot_result(res: dict, team1: str, team2: str, neutral: bool):
    venue = "Neutral venue" if neutral else f"{team1} home"
    mat = res["score_mat"]

    fig, axes = plt.subplots(1, 2, figsize=(13, 5.5),
                             gridspec_kw={"width_ratios": [2, 3]})

    # --- Balkendiagramm Sieg/Unentschieden/Niederlage ---
    ax = axes[0]
    labels = [f"{team1}\nwins", "Draw", f"{team2}\nwins"]
    ...
    im = ax.imshow(mat.T, origin="lower", cmap="Blues",
                   vmin=0, vmax=mat.max())
    ax.set_xlabel(f"{team1} goals", fontsize=11)
    ax.set_ylabel(f"{team2} goals", fontsize=11)
    ax.set_xticks(range(MAX_GOALS + 1))
    ax.set_yticks(range(MAX_GOALS + 1))
    ax.set_title("Most likely scorelines", fontsize=12)
    ...
    # Diagonale (Unentschieden) markieren
    ...
    fig.suptitle(f"Bayesian match prediction  |  {len(samples['attack'])} posterior samples",
                 fontsize=13, y=1.01)
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    plot_result(res, TEAM1, TEAM2, NEUTRAL_VENUE)

...
Vor dem Spawn der Worker wird der Numba-JIT im Hauptprozess einmal
"warmgelaufen" (cache=True), damit jeder Sub-Prozess den kompilierten Code
aus dem Cache lädt statt selbst zu kompilieren.
"""
...
def gelman_rubin(chains_values: list[np.ndarray]) -> float:
    ...
    R-hat < 1.05 gilt als konvergiert.
    """
...
def run_mcmc_parallel(L, n_chains: int = 4, n_iter_per_chain: int = 5000,
    ...
    Rückgabe-Keys:
      attack, defense, delta  — kombinierte Samples (wie run_mcmc)
      chains                  — Rohergebnisse pro Kette (für eigene Diagnostik)
      acceptance              — mittlere Akzeptanzrate
      rhat_max, rhat_mean     — Gelman-Rubin R-hat über alle Teams/Parameter
    """
    # JIT vorkompilieren im Hauptprozess → cache=True wird von den Workern genutzt

...
def animate_strength_evolution(samples, L, save_path, top_k: int = 6,
    ...
    axes[0].set_title("Attack strength a(t)")
    axes[1].set_title("Defense strength d(t)")
    for ax in axes:
    ...
    fig.suptitle("Animated strength evolution", fontsize=13)
    fig.tight_layout()
...
def animate_season_table(L, save_path, fps: int = 6):
    ...
    def update(fi):
        ...
        title.set_text(f"Standings after {cutoff} of {n_total} matches")
...
def animate_mcmc_walk(samples, L, save_path, team_idx: int = 0,
    ...
    axes[0].set_title(f"MCMC trace for {L.teams[team_idx]}: attack strength")
    axes[1].set_title("Defense strength")
    axes[0].set_ylabel("a")
    axes[1].set_ylabel("d")
    axes[1].set_xlabel("Match index (chronological)")
